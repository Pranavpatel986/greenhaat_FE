...
const EmployeeList = () => {
    ...
    // for calling other component by onclick
    const navigate = useNavigate();

    // set the data all data we are fetching and handle
  ...
  return (
    <div className='container mx-auto my-8 overflow-auto'>
        <div className='h-12 px-4 '>
            <button onClick={() => navigate("/addEmployee")} className='rounded bg-slate-600 text-white px-6 py-2 font-semibold hover:bg-slate-800'>
                Add Employee
            </button>
        </div>
        <div className='flex shadow border-b'>
            <table className='min-w-full'>
                <thead className= 'bg-gray-100'>
                    <tr>
                        <th className='text-left font-medium text-gray-500 uppercase tracking-wider py-3 px-6'>EmployeeID</th>
